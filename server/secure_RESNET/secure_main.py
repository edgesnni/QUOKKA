...
from secure_resnet import SResNet
    ...
    * val_loader:   validation data loader..
    ...
    with crypten.no_grad():
        for i in range(len_loader):
            ...
            if isinstance(model, SResNet):
                model.eval()
                start = time.process_time()
                enc_model = model.encrypt(src=0)
                zeroed_data = comm.get().broadcast_obj(None, src=1)
                crypt_data = crypten.cryptensor(torch.empty(zeroed_data.size()), src=1)
                inner_time = enc_model(crypt_data)
