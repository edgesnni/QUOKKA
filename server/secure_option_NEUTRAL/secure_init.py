...
#from plaintext.eenet import eenet152
from secure_eenet import seenet18, seenet34, seenet50, seenet101, seenet152
from secure_eenet import seenet20, seenet32, seenet44, seenet56, seenet110
from secure_resnet import sresnet18, sresnet34, sresnet50, sresnet101, sresnet152
from secure_resnet import sresnet20, sresnet32, sresnet44, sresnet56, sresnet110
from secure_resnet import set_complexity
from flops_counter import flops_to_string, params_to_string
from collections import OrderedDict
from utils import parse_weights, parse_model_filename, create_empty_dictionary
#pylint: enable=W0611
...
def initializer():
    ...
    parser.add_argument('--multi-gpu',    action='store_true', default=False,
                                          help='enable multi-gpu training. Should always be False for secure inference.')
    ...
    parser.add_argument('--model',        type=str,   default='seenet20',
                                          choices=['seenet8',
    ...
    parser.add_argument('--examine',      action='store_true', default=False,
                                          help='saves a csv copy of model and its output in relevant folder. Only available for secure inference.')
    ...
    if args.model == 'seenet8':
        args.num_ee = 2

    if args.model[:6] == "sresnet":
        args.num_ee = 0
    ...
    if args.load_model is None:
        ...
        secure_object = _get_object(args.model)

        secure_model = secure_object(**kwargs)
        crypten.init()
        print("\n\nCrypten initialized.\nLOADING MODEL. . . .")
        
        weights = torch.load(args.load_model, weights_only=True, map_location=torch.device('cpu'))
        parse_model_filename(args.load_model, args)
        pruned_weights = parse_weights(weights)
        assert pruned_weights.keys() == secure_model.state_dict().keys()
        print("2. Loaded plaintext weights.")
        secure_model.load_state_dict(pruned_weights, strict=False)

        print(f"3. Loaded weights into Secure Model s{args.model}.")
        zeroed_weights = create_empty_dictionary(pruned_weights)
        comm.get().broadcast_obj(zeroed_weights, src=0)
        print(f"4. Sent zeroed model structure to Client.")
        try:
            secure_model = secure_model.to(args.device) #move to device.
    ...
    if use_cuda and torch.cuda.device_count() > 1 and args.multi_gpu:
        raise ValueError("Do not use multiple GPUs when secure, because the code set that up using pytorch.")
    
    return secure_model, args
