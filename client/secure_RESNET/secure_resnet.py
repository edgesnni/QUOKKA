...
class eeSResNet(cnn.Module):
    """Builds a ResNet like architecture with layer dimensions matching SEENet's full path.

    Arguments are
    * block:              Block function of the architecture either 'BasicBlock' or 'Bottleneck'.
    * layers_repetitions: A list of integers specifying the number of blocks in each stage.
                          For SEENet152, this would be [3, 8, 36, 3].
    ...
    * input_shape:        Input shape of the model (C, H, W).
    """
    def __init__(self, block, layers_repetitions, num_classes, input_shape, zero_init_residual=False, **kwargs):
        super(eeSResNet, self).__init__()
        channel, _, _ = input_shape
        self.input_shape = input_shape
        self.inplanes = 64 # Initial inplanes as in SEENet

        # Initial convolutional block (from SEENet's 'self.layers.append(cnn.Sequential(...))')
        self.conv_initial = cnn.Sequential(
            cnn.Conv2d(channel, 64, kernel_size=7, stride=2, padding=3, bias=False),
            cnn.BatchNorm2d(64),
            cnn.ReLU(),
            cnn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )

        # Build the stages dynamically based on layers_repetitions
        self.stages = cnn.ModuleList()
        planes_values = [64, 128, 256, 512] # These are the 'planes' values for each stage in ResNet
        strides = [1, 2, 2, 2] # Strides for each stage

        for i, num_blocks in enumerate(layers_repetitions):
            stride = strides[i]
            planes = planes_values[i]
            self.stages.append(self._make_layer(block, planes, num_blocks, stride=stride))
            # self.inplanes is updated within _make_layer

        # Final average pooling and fully connected layers
        self.avgpool = cnn.AdaptiveAvgPool2d(1)

        # The final linear layer's input_features should match the output of the last stage
        # For ResNet, the last stage's output planes * block.expansion
        self.exit_linear = cnn.Linear(self.inplanes, num_classes) # self.inplanes holds the final output channels
        self.exit_softmax = cnn.Softmax(dim=1)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        self.inplanes = planes * block.expansion # Update inplanes after the first block in the layer
    ...
    def forward(self, x):
        starttime = time.process_time() # Uncomment if you need timing
        ...
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.exit_linear(x)
        x = self.exit_softmax(x)
        endtime = time.process_time() # Uncomment if you need timing
        elapsedtime = endtime-starttime # Uncomment if you need timing
        return elapsedtime
...
class SResNet(cnn.Module):
    ...
    * zero_init_residual: Zero-initialize the last BN in each residual branch,
    ...
    def __init__(self, block, layers, num_classes, input_shape, zero_init_residual=False, **kwargs):
        ...
        self.exit_linear = cnn.Linear(self.inplanes, num_classes)
        self.exit_softmax = cnn.Softmax(dim=1)
