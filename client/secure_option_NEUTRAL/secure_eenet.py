...
from helpers import Secureconv1x1, Secureconv3x3, encrypt_compare
import random
import numpy as np
import io
import pandas as pd
...
class SEENet(cnn.Module):
    ...
    * input_shape:        Input shape of the model according to dataset. Should always be false because not implemented in Crypten.
    ...
    def forward(self, x, crypt_threshold):
        for idx, crypt_exit in enumerate(self.exits):
            starttime = time.process_time()
            crypt_stages = self.stages[idx] 
            x = crypt_stages(x) 
            enc_pred, enc_conf = crypt_exit(x)
            encrypt_compare(enc_conf, crypt_threshold, operator.gt)
            pred = enc_pred.get_plain_text()
            conf = enc_conf.get_plain_text()
            instruction_from_server = comm.get().broadcast_obj(None, src=0)
            if instruction_from_server == "stop":
                endtime = time.process_time()
                elapsedtime = endtime-starttime
                return pred, idx, self.cost[idx], elapsedtime
            else:
                pass
        ...
        return pred, len(self.exits), 1.0, elapsedtime2
